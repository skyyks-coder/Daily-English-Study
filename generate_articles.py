...
def load_weekly_articles():
    ...
    if not items and TODAY_FILE.exists():
        ...
        if fallback.get("summary") and not fallback.get("isWeeklyReview", False):
            items.append(fallback)
            print("아카이브가 없어 기존 today_news.json을 초기 복습자료로 사용합니다.")

    if not items:
        raise RuntimeError("Weekly Review에 사용할 학습 자료가 없습니다.")
...
Return valid JSON only with keys summary, vocab, shadowing, quizzes, rephraseTarget.
Requirements: summary 3 to 5 CEFR C1 sentences; exactly 5 vocabulary items with Korean meanings;
exactly 5 shadowing sentences; exactly 3 quizzes with exactly 3 options each and zero-based answer 0, 1, or 2;
exactly 2 rephrasing exercises; do not invent facts; do not use Markdown fences.
"""
    response = client.chat.completions.create(
        model="openai/gpt-4o-mini",
        messages=[
            {"role": "system", "content": "Create an accurate Weekly Review. Return valid JSON only."},
            {"role": "user", "content": prompt},
        ],
        response_format={"type": "json_object"},
        temperature=0.3,
    ...
    if not raw:
        raise RuntimeError("Weekly Review 응답이 비어 있습니다.")
    result = StudyMaterial.model_validate(json.loads(raw))
    if len(result.vocab) != 5 or len(result.shadowing) != 5 or len(result.quizzes) != 3 or len(result.rephraseTarget) != 2:
        raise RuntimeError("Weekly Review 항목 개수가 요구사항과 다릅니다.")
    for quiz in result.quizzes:
        if len(quiz.options) != 3 or quiz.answer not in (0, 1, 2):
            raise RuntimeError("Weekly Review Quiz 구조가 올바르지 않습니다.")
...
def main():
    ...
    if today.weekday() == 6:
        ...
        save_today_news(article, study, is_weekly_review=True)
    else:
        ...
        save_today_news(article, study, is_weekly_review=False)
